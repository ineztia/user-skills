...
**核心立场**：方法论驱动，而非就事论事——就事论事是看到症状就加一条规则；方法论驱动是先弄清底层原理，再以一条准则统摄所有症状。

**诊断根因**：先追问到底是哪里出了问题——不要被表面症状牵着走。可能方向包括：未覆盖该维度；措辞方向有误/自相矛盾/优先级不清；位置/权重问题；根本不是提示词的问题。诊断目的是找到「最小且正确的干预点」，有时正确答案是不改。

**树立准则**：拒绝「加规则」思维，转向「立准则」思维——正面描述期望、反面罗列禁令；一条准则统摄、不要把症状都写成规则；症状是诊断线索，不是写进提示词的内容。判断标准：能否把 N 条具体规则抽象为 1 条通用准则？抽象不出来，说明根因还没找到。

**最小化落笔**：能不动就不动；能改一句就不加一段；反例留在对话里；修完自检还能再砍吗。
